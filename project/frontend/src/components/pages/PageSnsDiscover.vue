...
const emit = defineEmits(['need-auth', 'open-profile'])
...
const hasAnyTopResult = computed(
  () => topUsers.value.length || topPosts.value.length || topHashtags.value.length
)
  ...
  <div class="sns-discover">
    ...
    <template v-if="isSearchMode">
      <div class="sns-discover__inner">
        <TabBar :tabs="CATEGORY_TABS" :active="activeCategory" pill @update:active="onCategoryChange" />
              ...
              <section v-if="topHashtags.length" class="sns-discover__section">
                ...
                <ul class="sns-discover__hashtag-list">
                  <li v-for="h in topHashtags" :key="h.tag">
                    <button type="button" @click="onHashtagSelect(h.tag)">
...
.sns-discover__hashtag-list li button {
  width: 100%;
  display: flex;
  align-items: center;
  justify-content: space-between;
