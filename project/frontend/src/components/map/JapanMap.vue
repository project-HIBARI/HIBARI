...
<template>
  <div
    style="position: relative; width: 100%; background: #0d0806; border: 1px solid #3a2a1a; overflow: hidden"
    :style="{ aspectRatio: `${W} / ${H}` }"
  >
    <svg :viewBox="`0 0 ${W} ${H}`" width="100%" height="100%" style="display: block">
      <defs>
        <pattern id="jpgrid" width="24" height="24" patternUnits="userSpaceOnUse">
          <path d="M 24 0 L 0 0 0 24" fill="none" stroke="rgba(201,169,97,0.06)" stroke-width="0.5" />
        </pattern>
        <pattern id="jpdots" width="6" height="6" patternUnits="userSpaceOnUse">
          <circle cx="3" cy="3" r="0.4" fill="rgba(201,169,97,0.08)" />
        </pattern>
      ...
      <g v-for="lat in [35, 40]" :key="'lat' + lat">
        <line
          ...
          opacity="0.4"
        />
        <text :x="24" :y="proj(lat, minLng).y - 4" font-family="'JetBrains Mono', monospace" font-size="8" fill="#c9a961" opacity="0.6">
          {{ lat }}°N
        </text>
      </g>
      <g v-for="lng in [135, 140]" :key="'lng' + lng">
        <line
          ...
          opacity="0.4"
        />
        <text :y="H - 22" :x="proj(minLat, lng).x + 4" font-family="'JetBrains Mono', monospace" font-size="8" fill="#c9a961" opacity="0.6">
          {{ lng }}°E
        </text>
      </g>
      ...
      />
      <circle v-for="(cd, i) in chainDots" :key="'cd' + i" :cx="cd[0]" :cy="cd[1]" r="1.5" fill="#3a2a1a" />

      <g stroke="#3a2a1a" stroke-width="0.5" fill="none" opacity="0.7">
      ...
      <g>
        <rect x="20" y="20" width="200" height="58" fill="#050302" stroke="#c9a961" stroke-width="0.8" />
        <text x="30" y="42" font-family="'Shippori Mincho', serif" font-size="16" font-weight="700" fill="#f4ede0" letter-spacing="3">ゆかりの地</text>
        <text x="30" y="58" font-family="'Playfair Display', serif" font-style="italic" font-size="10" fill="#c9a961" letter-spacing="2">PLACES · PILGRIMAGE</text>
        <text x="30" y="72" font-family="'JetBrains Mono', monospace" font-size="8" fill="#8a6a3a" letter-spacing="2">{{ places.length }} LOCATIONS</text>
      </g>

      <g :transform="`translate(${W - 70}, 70)`">
        <circle r="28" fill="#050302" stroke="#c9a961" stroke-width="0.8" />
        <path d="M 0,-20 L 5,0 L 0,20 L -5,0 Z" fill="#8b1a1a" />
        <path d="M 0,-20 L 5,0 L 0,0 Z" fill="#c9a961" />
        <text y="-32" text-anchor="middle" font-family="'Playfair Display', serif" font-size="11" font-style="italic" fill="#c9a961">N</text>
        <text y="42" text-anchor="middle" font-family="'Playfair Display', serif" font-size="9" font-style="italic" fill="#8a6a3a">S</text>
        <text x="-34" y="3" text-anchor="middle" font-family="'Playfair Display', serif" font-size="9" font-style="italic" fill="#8a6a3a">W</text>
        <text x="34" y="3" text-anchor="middle" font-family="'Playfair Display', serif" font-size="9" font-style="italic" fill="#8a6a3a">E</text>
      </g>

      <g :transform="`translate(40, ${H - 40})`">
        <line x1="0" y1="0" x2="100" y2="0" stroke="#c9a961" stroke-width="1" />
        <line x1="0" y1="-4" x2="0" y2="4" stroke="#c9a961" stroke-width="1" />
        <line x1="50" y1="-3" x2="50" y2="3" stroke="#c9a961" stroke-width="1" />
        <line x1="100" y1="-4" x2="100" y2="4" stroke="#c9a961" stroke-width="1" />
        <text x="0" y="16" font-family="'JetBrains Mono', monospace" font-size="9" fill="#c9a961">0</text>
        <text x="50" y="16" text-anchor="middle" font-family="'JetBrains Mono', monospace" font-size="9" fill="#c9a961">150</text>
        <text x="100" y="16" text-anchor="middle" font-family="'JetBrains Mono', monospace" font-size="9" fill="#c9a961">300 km</text>
      </g>
      ...
      >
        ...
        <path
          ...
          :fill="selected && selected.id === p.id ? '#c0342f' : '#8b1a1a'"
          stroke="#c9a961"
      ...
      <g v-if="selected && calloutBox()">
        ...
        <rect
          ...
          stroke-width="1"
        />
        <text :x="calloutBox().bx + 12" :y="calloutBox().by + 20" font-family="'Shippori Mincho', serif" font-size="14" font-weight="700" fill="#f4ede0">
          {{ selected.name }}
        </text>
        <text :x="calloutBox().bx + 12" :y="calloutBox().by + 38" font-family="'Noto Serif JP', serif" font-size="10" fill="#c9a961">
          {{ selected.area }}
        </text>
        <g v-if="selected.visitable === false">
          <rect :x="calloutBox().bx + calloutBox().boxW - 44" :y="calloutBox().by + 6" width="36" height="14" fill="#8b1a1a" />
          <text
